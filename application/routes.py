...
def courses(term=None):
    if term is None:
        term = "Spring 2019"
    classess = Course.objects.order_by("-courseID")
    return render_template("courses.html", courseData=classess, courses=True, term=term)
...
def enrollment():
    ...
    if courseID:
        if Enrollment.objects(user_id=user_id, courseID=courseID):
            ...
            flash(f"You are enrolled in {courseTitle}!", "sucess")
